...
def build_gradient_clipper(config: dict) -> Optional[Callable]:
    ...
    if "clip_grad_val" in config.keys():
        ...
        clip_grad_fun = lambda params: \
            nn.utils.clip_grad_value_(parameters=params, clip_value=clip_value)
...
def build_optimizer(config: dict, parameters: Generator) -> Optimizer:
    ...
    if optimizer_name == "adam":
        ...
        adam_eps = config.get("adam_eps", 1e-9)
        optimizer = torch.optim.Adam(parameters, weight_decay=weight_decay, lr=learning_rate, betas=adam_betas, eps=adam_eps)
...
def build_scheduler(
    ...
    Current options:
        ...
        - "noam": see `mowgli.builders.NoamScheduler`
        - "warmupexponentialdecay": see
          `mowgli.builders.WarmupExponentialDecayScheduler`
